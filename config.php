...
return [
     ...
     *
     * ⚠ صداقتِ وابستگی: IndexNow یک push است، پس فایروالِ هاست جلویش
     * را نمی‌گیرد. ولی خبردادن یعنی موتور بعداً می‌آید بخزد؛ اگر همان
     * فایروال به خزندهٔ موتور ۴۰۳ بدهد، نمایه‌سازی کامل نمی‌شود. یعنی
     * این فیچر کمک می‌کند ولی گلوگاه همچنان WAF است.
     *
     * این جایگزینِ sitemap نیست — sitemap مالِ Rank Math می‌ماند.
     */
    'indexnow' => [
        'enabled' => true,

        /*
         * نقطهٔ اشتراکیِ پروتکل: هر موتورِ عضو، ارسال را با بقیه به
         * اشتراک می‌گذارد، پس یک ارسال کافی است و نیازی به فهرست‌کردنِ
         * تک‌تکِ موتورها نیست.
         */
        'endpoints' => ['https://api.indexnow.org/indexnow'],

        /* ثانیه — ویرایشِ پشتِ‌هم در این بازه به یک ارسال تبدیل می‌شود */
        'debounce' => 60,

        /* سقفِ صفِ ماندگار، تا یک کرونِ خراب یک ‎option‎ی متورم نسازد */
        'max_queue' => 500,

        /* نشانی در هر درخواست؛ سقفِ خودِ پروتکل ۱۰۰۰۰ است */
        'batch' => 1000,
    ],

    /*
     * ‎wp zig3d doctor‎ و صفحهٔ «ابزارها ← ZIG3D Doctor».
     *
     * صفحهٔ ادمین رویِ این نصب ضروری است نه اختیاری: هاست اشتراکی است
     * و WP-CLI در دسترس نیست، پس تنها راهِ اجرا همان دکمه است.
     */
    'doctor' => [
        'admin_page' => true,

        /* ثانیه، برایِ هر درخواستِ بررسی — سقفِ زمانِ اجرا رویِ هاستِ اشتراکی کوتاه است */
        'timeout' => 8,
    ],
